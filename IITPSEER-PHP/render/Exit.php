...
if(validate($id) && auth($id, $name, $token)){
    ...
    if(!mysqli_connect_errno()){
        $resq1 = mysqli_query($link, "SELECT * from Register WHERE BuildingNo = '".$bno."' AND ExitTime = '0000-00-00 00:00:00' AND ID = '".$id."' AND NOT EntryTime = '0000-00-00 00:00:00' ORDER BY EntryTime DESC");
        if(mysqli_num_rows($resq1) > 0){
            // $resq2 = null;
            while($row1 = mysqli_fetch_array($resq1, MYSQLI_ASSOC)){
                $exitt = $row1['EntryTime'];
                mysqli_query($link, "UPDATE Register SET ExitTime = '".$time."', EntryTime = '$exitt' WHERE BuildingNo = '".$bno."' AND ID = '".$id."' AND EntryTime = '".$exitt."'");
            }
        }    
        else{
            $null = null;
            mysqli_query( $link, "INSERT INTO Register(ID, FullName, BuildingNo, ExitTime, EntryTime) VALUES('$id', '$name', '$bno', '$time', '$null')");
        }
